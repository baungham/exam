...
public class UserUpdateDialog extends JDialog {
	 ...
	 * @param userPanel 
	 */
	public UserUpdateDialog(final UserPanel userPanel, final IUserService service) {
			...
			getContentPane().add(buttonPane, BorderLayout.SOUTH);
			{
				...
				okButton.addActionListener(new ActionListener() {
					...
					public void actionPerformed(ActionEvent e) {
						User user = new User();
						user.setId(UserUpdateDialog.this.user.getId());
						...
						if(user.getId() == 1 && s.isValue()== false){// 系统管理员不能禁用
							JOptionPane.showMessageDialog(null, "系统内置管理员不禁用!");
							return;
						}
						
						ServiceMessage msg = service.update(user);
						JOptionPane.showMessageDialog(null, msg.getMessage()); 
						if(msg.isStatus()){
							UserUpdateDialog.this.setVisible(false);
							userPanel.initData();// 数据初始化
							UserUpdateDialog.this.reset();
						}
					}
				});
	...
	public void showInfo(int id) {
		user = service.findById(id);
		 ...
		 editorPane.setText(user.getDesc()); 
