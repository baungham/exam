 ...
 * @version 1.0
 */
public class CertificateAddDialog extends JDialog {
	
	// 日志记录器
	protected Logger log = LoggerFactory.getLogger(CertificateAddDialog.class);
	...
	// 选择企业
	private EnterpriseSelectDialog esdialog = new EnterpriseSelectDialog(this);
	 ...
	 */
	public CertificateAddDialog(final CertificatePanel certificatePanel, final ICertificateService service) {
...
//		panel.setMinimumSize(new Dimension(600,500));
		...
		// 默认添加一个申证单元
		panel.add( new CertificateUnitPanel(panel)); 
			...
			getContentPane().add(buttonPane, BorderLayout.SOUTH);
			{
				...
				okButton.addActionListener(new ActionListener( ) { 
					...
					public void actionPerformed(ActionEvent e) {
						...
						ce.setProduct(textField_product.getText());// 产品名称
						ce.setCode(textField_code.getText());// 证书编码
						ce.setVerifyMode(textField_vmode.getText());// 检验方式
						ce.setEnterpriseId(emterpriseId);// 设置企业ID
						String stime = textField_sTime.getText();// 发证时间
						String etime = textField_eTime.getText();// 有效期至
						 
						try {
							ce.setSendTime(sdf.parse(stime));
							ce.setEffectiveTime(sdf.parse(etime));
						...
						for(int i=0; i<cs.length;i++){
							...
							ce.addUnit(cer.getUnit());
						}
						
						ServiceMessage msg = service.save(ce);
